...
abstract class FUtente
{
   ...
   public static function store(EUtente $utente) : bool
   {
       ...
       $sql = "INSERT INTO `Utente` (`Nome`, `Cognome`, `NomeUtente`, `Email`, `Telefono`, `Password`, `Punti`, `OrdiniCumulati`, `DataUltimoOrdine`) VALUES ('" . addslashes("$Nome") . "' , '" . addslashes("$Cognome") . "' , '$NomeUtente' , '$Email' , '$Telefono' , '$Password' , ':Punti' , ':OrdiniCumulati' , ':DataUltimoOrdine')";
...
//if ($control=== FALSE) $utente = FUtente::store($prova);
//if ($control == 1) echo "esiste"; else if ($control == 0) echo "non esiste";
//$control = FUtente::exists('giacpall');
